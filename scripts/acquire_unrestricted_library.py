...
UA='ProphetBiographyLibrary/6.8'
...
def gutenberg_text_url(x):
 """Resolve the current official UTF-8/plain-text file from the Gutenberg item page."""
 if str(x.get('sourceRepository') or '').lower()!='project gutenberg': return ''
 source=str(x.get('sourceUrl') or '').strip()
 ident=str(x.get('sourceIdentifier') or '').strip()
 if not safe_url(source) or not ident.isdigit(): return ''
 try:
  page,_,final=get(source,timeout=60)
  html=page.decode('utf-8','replace')
 except Exception:
  return ''
 patterns=[
  r'href=["\']([^"\']*?/ebooks/'+re.escape(ident)+r'\.txt\.utf-8)["\']',
  r'href=["\']([^"\']*?/cache/epub/'+re.escape(ident)+r'/pg'+re.escape(ident)+r'\.txt)["\']',
  r'href=["\']([^"\']+\.txt(?:\?[^"\']*)?)["\']',
 ]
 for pat in patterns:
...
def download_candidate(x,kind):
 urls=[]
 first=str(x.get('downloadUrl') or '').strip()
 if first: urls.append(first)
 fallback=gutenberg_text_url(x) if kind=='txt' else ''
 if fallback and fallback not in urls: urls.append(fallback)
 errors=[]
 for u in urls:
  if not safe_url(u):
   errors.append('domain-not-allowlisted'); continue
  try:
   b,ct,final=get(u)
  except Exception as e:
   errors.append('download:'+type(e).__name__); continue
...
def main():
 ...
 for x in c.get('items',[]):
  ...
  if not safe_url(x.get('downloadUrl','')) and not (str(x.get('sourceRepository') or '').lower()=='project gutenberg' and safe_url(x.get('sourceUrl',''))):
  ...
  kind=str(x.get('format') or 'txt').lower()
  b,ct,downloaded_url,error=download_candidate(x,kind)
  if b is None:
   row['status']='FAILED'; row['error']=error; keep(row); failed+=1; continue
  ...
  (ed/'provenance.json').write_text(json.dumps({'sourceRepository':x.get('sourceRepository'),'sourceIdentifier':x.get('sourceIdentifier'),'sourceUrl':x.get('sourceUrl'),'requestedDownloadUrl':x.get('downloadUrl'),'downloadUrl':downloaded_url,'retrievalDate':now,'originalFilename':orig.name,'sha256':digest,'size':len(b),'mimeType':meta['mimeType']},ensure_ascii=False,indent=2)+'\n',encoding='utf8')
  row.update({'status':'DOWNLOADED','resolvedDownloadUrl':downloaded_url,'localPath':str(orig.relative_to(ROOT)).replace('\\','/'),'sha256':digest,'editionId':eid,'error':''}); keep(row); acquired+=1
  if acquired>=a.limit: break
 state={'schema':'strict-acquisition-v3','updatedAt':now,'publishedWorkIdsKnown':len(published),'items':list(history.values())}
