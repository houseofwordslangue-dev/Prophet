...
EXPANDED=ROOT/'data'/'expanded_biographies_135_full.json'
...
def canonical_source_map():
 out={};x=read(EXPANDED,{}) or {}
 for r in x.get('people',[]) if isinstance(x,dict) else []:
  if not isinstance(r,dict) or not r.get('id'):continue
  src=r.get('source') if isinstance(r.get('source'),dict) else {}
  out[str(r['id'])]={'url':str(src.get('url') or ''),'hash':fp(r.get('biographyAr') or '') if r.get('biographyAr') else ''}
 return out
def main():
 audit=read(A,{}) or {};idx=read(I,{}) or {};idx.setdefault('people',{});O.mkdir(parents=True,exist_ok=True);res=list(audit.get('residualUnder500') or []);added_pages=added_words=completed=duplicates_skipped=0;canon=canonical_source_map()
 for r in res:
  ...
  original=canon.get(pid,{})
  for v in variants(name):
   page=get('سير أعلام النبلاء/'+v)
   if not page:continue
   if not readable(page['text']) or not match(v,page['title'],page['text']):continue
   k=fp(page['text'])
   if k in seen or (original.get('hash') and k==original['hash']) or (original.get('url') and page['url']==original['url']):
  ...
  if p.get('passages'):
   added=sum(int(x.get('wordCount') or wc(x.get('text') or '')) for x in p['passages']);final=before+added;status='EXTENDED_TO_500' if final>=TARGET else 'SOURCE_LIMITED';p.update({'schema':'short-biography-source-extension-v5','generatedAt':now(),'addedWords':added,'finalWords':final,'status':status,'policy':'Direct Siyar biography pages and prior direct source-owned extracts only; duplicate canonical source text excluded; no incidental mentions or invented factual fill-in.'});path.parent.mkdir(parents=True,exist_ok=True);path.write_text(json.dumps(p,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');idx['people'][pid]={'id':pid,'nameAr':name,'beforeWords':before,'addedWords':added,'finalWords':final,'status':status,'file':str(path.relative_to(ROOT))};
 ...
 new=[x for x in res if int(x.get('finalWords') or 0)<TARGET];audit.update({'schema':'short-biographies-500-audit-v5','generatedAt':now(),'directSiyarPagesAdded':added_pages,'directSiyarWordsAdded':added_words,'duplicateCanonicalSiyarPagesSkipped':duplicates_skipped,'extendedTo500':int(audit.get('extendedTo500') or 0)+completed,'sourceLimitedAfter':len(new),'residualUnder500':new,'complete':len(new)==0});idx.update({'schema':'short-biography-source-extension-index-v5','generatedAt':now()});I.write_text(json.dumps(idx,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');A.write_text(json.dumps(audit,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(json.dumps({k:audit.get(k) for k in ('indexedPeople','below500Before','extendedTo500','sourceLimitedAfter','directSiyarPagesAdded','directSiyarWordsAdded','duplicateCanonicalSiyarPagesSkipped','complete')},ensure_ascii=False))
