...
def chunks(text,source,locator=''):
    ...
    words=text.split(); out=[]; seen_spans=set(); hit_positions=[]
    ...
    for pos in hit_positions:
        ...
        if end-start<MIN_WORDS: start=max(0,end-MIN_WORDS)
        ...
        if ext in {'.txt','.md'}: rows+=chunks(p.read_text(encoding='utf-8',errors='ignore'),src,str(p.name))
            ...
            with zipfile.ZipFile(p) as z:
                for name in z.namelist():
                    if name.lower().endswith(('.xhtml','.html','.htm')): rows+=chunks(strip_html(z.read(name).decode('utf-8','ignore')),src,name)
        elif ext=='.pdf':
...
def drive_cache_sources():
    rows=[]; registry={}
    if DRIVE_MANIFEST.exists(): registry={s['id']:s for s in json.loads(DRIVE_MANIFEST.read_text(encoding='utf-8')).get('sources',[])}
...
def dedupe(rows):
    ...
    for r in rows:
        ...
        if any(SequenceMatcher(None,k[:3500],prev[:3500]).ratio()>=0.72 for prev in fingerprints[-800:]): continue
...
def main():
    ...
    articles=[article(r,i+1) for i,r in enumerate(rows[:TARGET])]
    ...
    for a in articles:
        cats[a['category']]+=1; sections[a['section']]=sections.get(a['section'],0)+1; ch=a['sources'][0]['sourceChannel']; channels[ch]=channels.get(ch,0)+1
