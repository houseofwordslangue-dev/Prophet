...
def source_meta(row: dict, rel: str):
    ...
    meta = {
        'repositoryPath': rel,
        'recordId': row.get('id'),
        'recordTitle': row.get('title'),
        'sourceType': 'explicit-person-owned-editorial-extraction',
    }
    if isinstance(sources, list) and sources:
        meta['sources'] = sources[:8]
    if row.get('sourceRefs'):
        meta['sourceRefs'] = row.get('sourceRefs')[:40] if isinstance(row.get('sourceRefs'), list) else row.get('sourceRefs')
...
def main():
    prior_index = read_json(INDEX_PATH, {}) or {}
    candidates = defaultdict(list)
    person_names = {}
    person_categories = {pid: meta.get('category') for pid, meta in (prior_index.get('people') or {}).items()}
    seen = defaultdict(set)
    source_records_scanned = 0
    explicit_source_records = 0
    ...
    for path in sorted(DRAFT_ROOT.glob('**/*.json')):
        ...
        for row in rows_of(payload):
            ...
            person = explicit_person(row)
            if not person or not source_backed(row):
                continue
            explicit_source_records += 1
            pid, name = person
            person_names.setdefault(pid, name)
            priority = 0 if life_intent(row) else 1
            meta = source_meta(row, rel)
    ...
    for stale in EXT_DIR.glob('*.json'):
        stale.unlink()
    ...
    for pid, rows in sorted(candidates.items()):
        rows.sort(key=lambda x: (x['_priority'], -len(x['text'])))
        capped_away += max(0, len(rows) - MAX_PASSAGES_PER_PERSON)
        ...
        payload = {
            'schema': 'canonical-biography-source-extension-v2',
            'personId': pid,
            'personNameAr': person_names.get(pid, pid),
            'policy': 'Only source passages from records explicitly linked to this exact person; no name-only or cross-person enrichment.',
            'passageCount': len(kept),
            'passages': kept,
        }
        (EXT_DIR / fname).write_text(json.dumps(payload, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
        words = sum(x['wordCount'] for x in kept)
        total_passages += len(kept)
        total_words += words
        people[pid] = {
            'id': pid,
            'nameAr': person_names.get(pid, pid),
            'category': person_categories.get(pid),
            'passageCount': len(kept),
            'wordCount': words,
            'file': relfile,
        }
    ...
    audit['biographyExtensionOwnershipPolicy'] = 'explicit-person-link-before-ranking-and-cap'
    audit['sourceRecordsScannedForStrictExtensions'] = source_records_scanned
    audit['explicitPersonOwnedSourceRecords'] = explicit_source_records
    audit['peopleExtendedAfterStrictRebuild'] = len(people)
    audit['extensionPassagesAfterStrictRebuild'] = total_passages
    audit['extensionWordsAfterStrictRebuild'] = total_words
    audit['candidatePassagesCappedAfterOwnershipCheck'] = capped_away
    ...
    print(json.dumps({
        ...
        'words': total_words,
        'explicitSourceRecords': explicit_source_records,
        'cappedAfterOwnership': capped_away,
        'aliPresent': 'ali-ibn-abi-talib' in people,
        'fatimaPresent': 'fatima-al-zahra' in people,
        'khadijaPresent': 'khadija-bint-khuwaylid' in people,
    }, ensure_ascii=False))

    if not people or not total_passages:
        raise SystemExit('Strict biography rebuild produced no source extensions')
