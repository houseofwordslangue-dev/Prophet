...
def assembled():
    ...
    for x in allb.get('people') or []:
        ...
        if len(hits)==1:p=hits[0]
            ...
            p={'id':x.get('id'),'name':{'ar':x.get('nameAr'),'en':x.get('nameAr'),'fr':x.get('nameAr')},'category':'family','sourcePassages':[]}
            mp[p['id']]=p
        apply_bio(p,x)
...
def rendered_has_content(p,lang):
    pb=p.get('professionalBiography') or {}
    bio=pb.get(lang) or pb.get('ar') or []
    if nonempty_seq(bio):return True,'professionalBiography'
    passages=[x for x in (p.get('sourcePassages') or []) if (x.get('language') or 'ar')==lang and str(x.get('text') or '').strip()]
    mentions=[x for x in passages if x.get('relation')!='authored-saying']
    if mentions:return True,'sourcePassages'
    return False,'empty'

def main():
    people,chunk_error=assembled()
    required=[];reference=[];langs=['ar','en','fr'];category_counts={}
    for p in people:
        cat=p.get('category') or 'unknown';category_counts[cat]=category_counts.get(cat,0)+1
        missing=[l for l in langs if not rendered_has_content(p,l)[0]]
