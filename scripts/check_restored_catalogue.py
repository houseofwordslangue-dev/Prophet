...
chunks=manifest.get('fallbackChunks') or manifest.get('chunks') or []
assert chunks, 'no fallback catalogue chunks declared'
rows=[]
for chunk in chunks:
    ...
    rows.extend(items)
assert len(rows)==manifest['baselineCount']==689, f"restored fallback baseline count mismatch: {len(rows)}"
ids=[str(r[0]) for r in rows]
assert len(set(ids))==len(ids)==689, 'restored fallback contains duplicate ids'
...
assert 'capabilities:{readable:false,searchable:false,listenable:false,watchable:false}' in loader, 'historical records must not expose fake public actions'
...
print(f'PASS fallback: restored={len(ids)}, uploaded={len(published_ids)}, overlap={overlap}, live_unique={union}')
