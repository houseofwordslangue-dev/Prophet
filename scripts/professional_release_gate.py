...
REQUIRED_CHILDREN=[
 ROOT/'data/children/taxonomy.json',ROOT/'data/children/media-sources.json',
 ROOT/'data/children/stories/manifest.json',ROOT/'data/children/stories/index.json',
 ROOT/'data/children/animated/manifest.json',ROOT/'data/children/animated/index.json',ROOT/'data/children/animated/status.json',
 ROOT/'data/children/very-short/index.json',ROOT/'data/children/very-short/status.json']
...
def main():
    ...
    for page in CRITICAL_PAGES:
        ...
        styles=re.findall(r'<link\b[^>]*rel=["\']stylesheet["\'][^>]*>',text,re.I)
    ...
    illustrated=load(ROOT/'data/children/stories/manifest.json') or {}
    animated=load(ROOT/'data/children/animated/status.json') or {}
    veryshort=load(ROOT/'data/children/very-short/status.json') or {}
    sources=load(ROOT/'data/children/media-sources.json') or {}
    ...
    if len(taxonomy.get('subjects') or [])<12:warnings.append(f"children taxonomy subject coverage below current baseline: {len(taxonomy.get('subjects') or [])}/12")
