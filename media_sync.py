...
YouTube embeds. Transcripts used for editorial grounding are handled separately.
"""
from pathlib import Path
from urllib.request import Request,urlopen
from urllib.parse import urljoin,urlparse
from html.parser import HTMLParser
import json,hashlib,datetime,os,re,threading
ROOT=Path(__file__).resolve().parent
CONFIG=ROOT/'private'/'media_sources.json'; OUT=ROOT/'data'/'imported_media.json'; STATE=ROOT/'private'/'media_sync_state.json'
...
def category(src,e=None):return str(src.get('category') or (src.get('topics') or ['lecture'])[0] or 'lecture')
def normalize_entry(e,src):
 ...
 return {'id':'yt-'+vid,'kind':'video','category':category(src,e),'titleEn':title,'titleAr':title,'titleFr':title,'summaryEn':'','summaryAr':'','summaryFr':'','url':f'https://www.youtube.com/watch?v={vid}','embed':yt_embed(vid),'language':src.get('language','ar'),'topics':cats,'transcriptStatus':'external','duration':str(e.get('duration_string') or e.get('duration') or 'video'),'chapters':[],'thumbnail':thumb,'published':e.get('upload_date') or e.get('release_date') or '', 'creator':creator,'series':str(src.get('series') or src.get('label') or '')[:240],'venue':str(src.get('venue') or '')[:240],'sourceType':src.get('kind',''),'catalogSourceId':src.get('id',''),'catalogTier':'official' if src.get('kind') in {'youtube','youtube-single','web-media'} else 'repertory-search','viewCount':int(e.get('view_count') or 0) if str(e.get('view_count') or '').isdigit() else 0}
...
def sync_all(max_entries=0):
 with LOCK:
  cfg=load(CONFIG,{'sources':[]});prev=load(OUT,{'items':[]});by={x['id']:x for x in prev.get('items',[]) if x.get('id')}
