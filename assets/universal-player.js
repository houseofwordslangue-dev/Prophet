...
function playItem(item,host){
 if(!host)return;
 host.innerHTML='';
 var local=item.localUrl||item.video||item.audio||'';
 if(local){var tag=(item.kind==='audio'||item.medium==='audio')?'audio':'video';var el=document.createElement(tag);el.controls=true;el.preload='metadata';el.src=local;if(item.thumbnail&&tag==='video')el.poster=item.thumbnail;host.appendChild(el);enhance(el);el.play().catch(function(){});return;}
 var note=document.createElement('div');note.className='media-external-note';note.innerHTML='<strong>المادة غير محفوظة محلياً بعد.</strong><br>يعمل المشغل المحلي فقط مع الملفات المرخّصة أو المملوكة أو المسموح بحفظها محلياً.';host.appendChild(note);
 if(item.embed){var f=document.createElement('iframe');f.src=item.embed;f.loading='lazy';f.allow='accelerometer; autoplay; encrypted-media; picture-in-picture; fullscreen';f.referrerPolicy='strict-origin-when-cross-origin';f.title=item.titleAr||item.titleEn||'Media';host.appendChild(f)}
}
